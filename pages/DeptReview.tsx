...
const DeptReview: React.FC = () => {
  ...
  const deptProposals = proposals.filter(p =>
    ...
    (p.targetDepartment === currentUser.department || currentUser.role === 'Admin') // Admins can see all
  );
          ...
          deptProposals.map(proposal => (
            <div key={proposal.id} className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden flex flex-col md:flex-row">
              <div className="p-6 flex-1">
                ...
                <p className="text-slate-600 text-sm leading-relaxed mb-4 bg-slate-50 p-4 rounded-lg border border-slate-100 whitespace-pre-line">
                  {proposal.summary}
                </p>
