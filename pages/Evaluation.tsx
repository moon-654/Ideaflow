...
import { Search, Filter, Save, Layers, Star, CheckCircle, AlertOctagon, ArrowRight, Award } from 'lucide-react';
...
const Evaluation: React.FC = () => {
  ...
  const [localProposals, setLocalProposals] = useState<Proposal[]>(MOCK_PROPOSALS);
  ...
  const handle1stScoreChange = (id: string, field: 'necessity' | 'feasibility', val: string) => {
    ...
    setLocalProposals(prev => prev.map(p => {
      ...
      return { ...p, score1st: newScore };
    }));
  };

  const submit1stReview = (id: string) => {
    // In real app: API call to move status
    const p = localProposals.find(item => item.id === id);
    if(p?.score1st?.passed) {
       alert(`제안 ${id} 건이 2차 심의로 상정되었습니다! (+2 마일리지)`);
       setLocalProposals(prev => prev.map(item => item.id === id ? {...item, status: '2nd_Review'} : item));
    } else {
       alert(`제안 ${id} 건이 기준 점수 미달로 탈락 처리되었습니다. (80점 미만)`);
       setLocalProposals(prev => prev.map(item => item.id === id ? {...item, status: 'Rejected'} : item));
    }
  };
  ...
  const handle2ndGradeChange = (id: string, grade: 'S'|'A'|'B'|'C') => {
    setLocalProposals(prev => prev.map(p => {
      if (p.id !== id) return p;
      return { ...p, grade2nd: grade };
    }));
  };

  const submit2ndReview = (id: string) => {
     const p = localProposals.find(item => item.id === id);
     if(!p?.grade2nd) return alert("등급을 선택해주세요.");
     
     alert(`제안 ${id} 건이 최종 ${p.grade2nd}등급으로 확정되었습니다! (포상 정산 대기)`);
     setLocalProposals(prev => prev.map(item => item.id === id ? {...item, status: 'Completed'} : item));
  };
